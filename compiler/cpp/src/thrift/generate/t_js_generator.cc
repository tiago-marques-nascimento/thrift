...
string t_js_generator::render_react_consts() {
  return string(
    ...
    "        <Flex justifyContent='start' alignItems='baseline' direction='column' flex='grow'>\n"
...
void t_js_generator::get_react_state_and_use_effect(t_struct *tstruct, t_field* tfield) {
  ...
  if (
    ...
    if (
      ...
      f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<any[] | undefined>(prop" << tstruct->get_name() << "." << (tfield)->get_name() << "?.map((it: any) => " << convert_it_to_string() << "));\n";
      f_react_ts_ << ts_indent() << "useEffect(() => {\n";
      indent_up();
...
void t_js_generator::get_react_initial_use_effect(t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (
      ...
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << "?.map((it: any) => " << convert_it_to_string() << "));\n";
    } else if (
      true_member_type->is_struct()
    ) {
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << ");\n";
