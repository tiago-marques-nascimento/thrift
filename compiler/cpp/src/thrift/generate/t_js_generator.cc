...
string t_js_generator::render_react_consts() {
  return string(
    ...
    "type ThriftInputProps = { value: any, setValue: Function, readonly: boolean, key: string, label: string };\n"
    "type ThriftSelectListInputProps = { list: any[], value: any, setValue: Function, readonly: boolean, key: string, label: string };\n"
    "type ThriftStructInputProps = { add: Function, remove?: Function, children: any, value: any, readonly: boolean, key: string, label: string };\n"
    "type ThriftChunkInputProps = { list: any[], renderChunkList: Function, key: string, label: string };\n"
    "type ThriftListInputProps = { add: Function, remove: Function, list: any[], renderListItem: any, readonly: boolean, key: string, label: string };\n"
    "type ThriftMapInputProps = { add: Function, remove: Function, list: any[], renderMapKey: any, renderMapValue: any, readonly: boolean, key: string, label: string };\n"

    "const ThriftInputSwitch = ({ value, setValue, readonly, key, label }: ThriftInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    "      </Label>\n"
    "      <Switch\n"
    ...
    "const ThriftInputNumberField = ({ value, setValue, readonly, key, label }: ThriftInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputTextField = ({ value, setValue, readonly, key, label }: ThriftInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputTextArea = ({ value, setValue, readonly, key, label }: ThriftInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputSelectList = ({ list, value, setValue, readonly, key, label }: ThriftSelectListInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputStruct = ({ add, remove, children, value, readonly, key, label }: ThriftStructInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputChunk = ({ list, renderChunkList, key, label }: ThriftChunkInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputList = ({ add, remove, list, renderListItem, readonly, key, label }: ThriftListInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "const ThriftInputMap = ({ add, remove, list, renderMapKey, renderMapValue, readonly, key, label }: ThriftMapInputProps) => {\n"
    "  return (\n"
    "    <Flex.Item flex='grow' key={key}>\n"
    "      <Label htmlFor={label}>\n"
    "        <Text>{label}</Text>\n"
    ...
    "                            <Flex justifyContent='start' alignItems='baseline' direction='column' gap={6}>\n"
    "                              <Flex.Item flex='grow'>\n"
    "                                <Label htmlFor={`${label}_key[${i}]`}>\n"
    "                                  <Text>{`${label}_key[${i}]`}</Text>\n"
    "                                </Label>\n"
    "                                <Box justifyContent='start' alignItems='baseline'>\n"
    "                                  <Flex justifyContent='start' alignItems='baseline' direction='column' gap={6}>\n"
    "                                    {renderMapKey(item, i)}\n"
    ...
    "                              </Flex.Item>\n"
    "                              <Flex.Item flex='grow'>\n"
    "                                <Label htmlFor={`${label}_value[${i}]`}>\n"
    "                                  <Text>{`${label}_value[${i}]`}</Text>\n"
    "                                </Label>\n"
    "                                <Box justifyContent='start' alignItems='baseline'>\n"
    "                                  <Flex justifyContent='start' alignItems='baseline' direction='column' gap={6}>\n"
    "                                    {renderMapValue(item, i)}\n"
    "                                  </Flex>\n"
    "                                </Box>\n"
    "                              </Flex.Item>\n"
    "                              <Flex.Item flex='grow'>\n"
    "                                <Button\n"
    "                                  disabled={readonly || !remove}\n"
    "                                  text='Remove Item'\n"
    "                                  size='sm'\n"
    "                                  color='transparent'\n"
    "                                  onClick={() => {remove(item)}}\n"
  ...
  if (
    ...
    if (
      ...
      if (((t_base_type *)(true_member_type))->get_base() == t_base_type::TYPE_I64) {
        ...
        f_react_ts_ << ts_indent() << "const update" << capitalize(member_name) << " = (value: string) => { set" << capitalize(member_name) << "(value) };\n\n";
      } else if (
        ...
        f_react_ts_ << ts_indent() << "}, [" << member_name << "]);\n";
        f_react_ts_ << ts_indent() << "const update" << capitalize(member_name) << " = (value: number) => { set" << capitalize(member_name) << "(value) };\n\n";
      } else if (
      ...
      f_react_ts_ << ts_indent() << "}, [" << member_name << "]);\n";
      f_react_ts_ << ts_indent() << "const update" << capitalize(member_name) << " = () => { set" << capitalize(member_name) << "([..." << member_name << "]) };\n\n";
      ...
      f_react_ts_ << ts_indent() << "}, [" << member_name << "]);\n";
      f_react_ts_ << ts_indent() << "const update" << capitalize(member_name) << " = (value?: " << find_include_2_import_name(true_member_type->get_program()) << "_module." << true_member_type->get_name() << ") => { set" << capitalize(member_name) << "({..." << member_name << "} as " << find_include_2_import_name(true_member_type->get_program()) << "_module." << true_member_type->get_name() << " | undefined) };\n\n";
    }
  }
}
...
void t_js_generator::get_react_component(string member_name, string key_name, bool is_optional, bool use_setter, string parent_member, t_type *member_type, t_program *program, t_struct* tstruct) {
  ...
  if (
    ...
    if (
      ...
      if (use_setter) {
        ...
        f_react_ts_  << ts_indent() << "  setValue={() => {\n"
                     ...
                     << ts_indent() << "    update" << capitalize(parent_member) << "();\n"
                     << ts_indent() << "  }}\n";
      }

      f_react_ts_ << ts_indent() << "  readonly={readonly}\n"
                  << ts_indent() << "  key={" << key_name << "}\n"
                  << ts_indent() << "  label={'" << member_name << "'}\n"
                  << ts_indent() << "/>\n";
      ...
      if (use_setter) {
        f_react_ts_  << ts_indent() << "  setValue={(e: any) => { update" << capitalize(member_name) << "(Number(e.value)) }}\n";
      } else {
        f_react_ts_  << ts_indent() << "  setValue={(e: any) => {\n"
                     << ts_indent() << "    " << member_name << " = Number(e.value);\n"
                     << ts_indent() << "    update" << capitalize(parent_member) << "();\n"
                     << ts_indent() << "  }}\n";
      }
      ...
      if (use_setter) {
        f_react_ts_  << ts_indent() << "  setValue={(e: any) => { update" << capitalize(member_name) << "(e.value as string) }}\n";
      } else {
        f_react_ts_  << ts_indent() << "  setValue={(e: any) => {\n"
                     << ts_indent() << "    " << member_name << " = e.value as string;\n"
                     << ts_indent() << "    update" << capitalize(parent_member) << "();\n"
                     << ts_indent() << "  }}\n";
      }
    ...
    f_react_ts_ << ts_indent() << "<ThriftInputChunk\n"
                << ts_indent() << "  list={groupByChunk(" << member_name << ")}\n"
    ...
    if (use_setter) {
      f_react_ts_ << ts_indent() << "    update" << capitalize(member_name) << "(" << new_react_object(true_member_type) + ");\n";
    } else {
    ...
    if (use_setter) {
      f_react_ts_ << ts_indent() << "    update" << capitalize(member_name) << "(undefined);\n";
    } else {
    ...
    if (use_setter) {
      f_react_ts_ << ts_indent() << "    prop" << true_member_type->get_name() << "={" << member_name << "!}\n"
                  << ts_indent() << "    setProp" << true_member_type->get_name() << "={update" << capitalize(member_name) << "}\n";
    } else {
      f_react_ts_ << ts_indent() << "    prop" << true_member_type->get_name() << "={ " << member_name << "! }\n"
                  << ts_indent() << "    setProp" << true_member_type->get_name() <<"={ (value: any) => {\n"
                  << ts_indent() << "      " << member_name << " = value;\n";
      
      f_react_ts_ << ts_indent() << "      update" << capitalize(parent_member) << "();\n"
                  << ts_indent() << "    }}\n";
    }
...
void t_js_generator::get_react_chunk_component(string member_name, string key_name, bool is_optional, bool use_setter, string parent_member, t_type *member_type, t_program *program, t_struct* tstruct) {
  ...
  if (member_type->is_list() || member_type->is_set()) {
    ...
    f_react_ts_ << ts_indent() << "  }}\n"
                << ts_indent() << "  remove={(i2: number) => {\n";

    f_react_ts_ << ts_indent() << "    " << member_name << " = " << member_name << ".filter((it: any, i: number) => i !== i2);\n"
                << ts_indent() << "    update" << capitalize(parent_member) << "();\n";
