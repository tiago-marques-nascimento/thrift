...
public:
  ...
  // (tmarquesdonascimento) parses field member name
  std::string parse_member_name(std::string member_name);

  // (tmarquesdonascimento) convert it from a .map lambda to string
  std::string convert_it_to_string();
...
string t_js_generator::render_react_includes() {
  ...
  result += string(
    ...
    "import { Box, Label, SelectList, Text, Link, Switch, Flex, Button, TextField, TextArea, Module, NumberField, IconButton } from 'gestalt';\n"
  );
...
string t_js_generator::render_react_consts() {
  return string(
    "import { CHUNK_LENGTH, ThriftInputProps, ThriftSelectListInputProps, ThriftStructInputProps, ThriftChunkInputProps, ThriftListInputProps, ThriftMapInputProps, ThriftInputBooleanType, booleanToThriftInputBooleanType, thriftInputBooleanTypeToBoolean, ThriftInputBoolean, ThriftInputNumberField, ThriftInputTextField, ThriftInputTextArea, ThriftInputSelectList, ThriftInputStruct, ThriftInputChunk, ThriftInputList, ThriftInputMap, fst, mts } from './../common/thriftFormUtils';"
    "import { parsePayloadFromThrift, parsePayloadToThrift, removeHiddenFieldsFromThriftData } from './../common/thriftUtils';"
  );
}
...
std::string t_js_generator::build_contained_prodecure(t_type* t, std::string it) {
  if (t->is_list() || t->is_set()) {
    ...
    return it + ".map(it => ({ key: " + build_contained_prodecure(((t_map*)t)->get_key_type()->get_true_type(), "it.key") + ", value: " + build_contained_prodecure(((t_map*)t)->get_val_type()->get_true_type(), "it.value") + ", __key__ttype: '" + get_member_type(((t_map*)t)->get_key_type()->get_true_type()) + "', __value__ttype: '" + get_member_type(((t_map*)t)->get_val_type()->get_true_type()) + "' }))";
  } else if (t->is_struct()) {
...
// (tmarquesdonascimento) parses field member name
std::string t_js_generator::parse_member_name(std::string member_name) {
  // (tmarquesdonascimento): add check to prevent use of protected arguments variable.
  return (member_name == "arguments") ? "argumentsInternal" : member_name;
}

// (tmarquesdonascimento) convert it from a .map lambda to string
std::string t_js_generator::convert_it_to_string() {
  return "(it instanceof Int64) ? (isFinite((it as Int64).toNumber()) ? `${(it as Int64).toNumber()}` : BigInt('0x' + (it as Int64).toOctetString()).toString()) : it";
}
...
void t_js_generator::get_react_state_and_use_effect(t_struct *tstruct, t_field* tfield) {
  string member_name = parse_member_name((tfield)->get_name());
  ...
  if (
    ...
    if (
      ...
      if (((t_base_type *)(true_member_type))->get_base() == t_base_type::TYPE_I64) {
        ...
        f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<string>(prop" << tstruct->get_name() << "." << (tfield)->get_name() << " != null ? BigInt('0x' + (prop" << tstruct->get_name() << "." << (tfield)->get_name() << ").toOctetString()).toString() : '');\n";
        ...
        f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<string>(prop" << tstruct->get_name() << "." << (tfield)->get_name() << " != null ? `${prop" << tstruct->get_name() << "." << (tfield)->get_name() << "}` : '');\n";
        f_react_ts_ << ts_indent() << "useEffect(() => {\n";
        indent_up();
        ...
        f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<ThriftInputBooleanType>(booleanToThriftInputBooleanType(prop" << tstruct->get_name() << "." << (tfield)->get_name() << "));\n";
        f_react_ts_ << ts_indent() << "useEffect(() => {\n";
        indent_up();
        ...
        f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<string>(prop" << tstruct->get_name() << "." << (tfield)->get_name() << " != null ? prop" << tstruct->get_name() << "." << (tfield)->get_name() << ".toString('hex') : '');\n";
        f_react_ts_ << ts_indent() << "useEffect(() => {\n";
        indent_up();
      ...
      f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<any[] | undefined>(mts(prop" << tstruct->get_name() << "." << (tfield)->get_name() << "));\n";
      f_react_ts_ << ts_indent() << "useEffect(() => {\n";
      indent_up();

      f_react_ts_ << ts_indent() << "prop" << tstruct->get_name() << "." << (tfield)->get_name() << " = "
                  << "!!" << member_name << " ? "
                  << "[..." << member_name << "] : "
      ...
      f_react_ts_ << ts_indent() << "const [" << member_name << ", set" << capitalize(member_name) << "] = useState<" << find_include_2_import_name(true_member_type->get_program()) << "_module." << true_member_type->get_name() << " | undefined>(prop" << tstruct->get_name() << "." << (tfield)->get_name() << ");\n";
      f_react_ts_ << ts_indent() << "useEffect(() => {\n";
      indent_up();
...
void t_js_generator::get_react_initial_use_effect(t_struct *tstruct) {
  ...
  for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
    ...
    if (
      ...
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << " ?? '');\n";
    } else if (
      true_member_type->is_list() ||
      true_member_type->is_set() ||
      true_member_type->is_map()
    ) {
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(mts(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << "));\n";
    } else if (
      true_member_type->is_struct()
    ) {
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << ");\n";
      ...
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(booleanToThriftInputBooleanType(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << "));\n";
    } else if (
      true_member_type->is_base_type() &&
      ((t_base_type *)(true_member_type))->get_base() == t_base_type::TYPE_I64
    ) {
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << " != null ? BigInt('0x' + (prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << ").toOctetString()).toString() : '');\n";
    } else if (true_member_type->is_binary()) {
      f_react_ts_ << ts_indent() << "set" << capitalize(member_name) << "(prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << " != null ? prop" << tstruct->get_name() << "." << (*m_iter)->get_name() << ".toString('hex') : '');\n";
    } else if (true_member_type->is_enum()) {
      string enum_type = find_include_2_import_name(true_member_type->get_program()) + "_module." + true_member_type->get_name();
      if (is_optional) {
...
string t_js_generator::new_react_object(t_type *member_type, bool first) {
  ...
  if (
    ...
    return first ? "{ key: " + new_react_object(((t_map *)true_member_type)->get_key_type(), false) + ", value: " + new_react_object(((t_map *)true_member_type)->get_val_type(), false) + ", __key__ttype: '" + get_member_type(((t_map *)true_member_type)->get_key_type()) + "', __value__ttype: '" + get_member_type(((t_map *)true_member_type)->get_val_type()) + "' }" : "[]";
  } else if (true_member_type->is_enum()) {
    return "''";
  } else if (true_member_type->is_struct()) {
    ...
    if (should_enable_react) {
      if (members_count <= member_max_allowed_count_for_react_component) {
        ...
        f_react_ts_ << ts_indent() << "<Box justifyContent='start' alignItems='baseline' padding={4}>\n"
                    ...
                    << ts_indent() << "    <Flex.Item flex='grow' alignSelf='start'>\n"

                    << ts_indent() << "      <Flex justifyContent='start' alignItems='center' direction='row'>\n"
                    << ts_indent() << "       <Flex.Item flex='grow' alignSelf='center'>\n"
                    ...
                    << ts_indent() << "       </Flex.Item>\n";
        if (members.size() > 0) {
          f_react_ts_ << ts_indent() << "       <Flex.Item flex='shrink' alignSelf='center'>\n"
                      << ts_indent() << "         <IconButton\n"
                      << ts_indent() << "           accessibilityLabel='Copy'\n"
                      << ts_indent() << "           icon='copy-to-clipboard'\n"
                      << ts_indent() << "           onClick={async () => {\n"
                      << ts_indent() << "             const processedPayload = removeHiddenFieldsFromThriftData(\n"
                      << ts_indent() << "               parsePayloadFromThrift(prop" << tstruct->get_name() << "),\n"
                      << ts_indent() << "             );\n"
                      << ts_indent() << "             try {\n"
                      << ts_indent() << "               await navigator.clipboard.writeText(JSON.stringify(processedPayload, null, 4));\n"
                      << ts_indent() << "             } catch (error) {\n"
                      << ts_indent() << "               console.error(`Error copying to clipboard: ${error}`);\n"
                      << ts_indent() << "             }\n"
                      << ts_indent() << "           }}\n"
                      << ts_indent() << "           size='sm'\n"
                      << ts_indent() << "           tooltip={{ text: 'Copy' }}\n"
                      << ts_indent() << "         />\n"
                      << ts_indent() << "       </Flex.Item>\n"
                      << ts_indent() << "       <Flex.Item flex='shrink' alignSelf='center'>\n"
                      << ts_indent() << "         <IconButton\n"
                      << ts_indent() << "           accessibilityLabel='Paste'\n"
                      << ts_indent() << "           icon='send'\n"
                      << ts_indent() << "           onClick={async () => {\n"
                      << ts_indent() << "             try {\n"
                      << ts_indent() << "               const processedPayload = JSON.parse(await navigator.clipboard.readText());\n"
                      << ts_indent() << "               setProp" << tstruct->get_name() << "(parsePayloadToThrift(new " << find_include_2_import_name(tstruct->get_program()) << "_module." << tstruct->get_name() << "(processedPayload), 'struct'));\n"
                      << ts_indent() << "             } catch (error) {\n"
                      << ts_indent() << "               console.error(`Error reading from clipboard: ${error}`);\n"
                      << ts_indent() << "             }\n"
                      << ts_indent() << "           }}\n"
                      << ts_indent() << "           size='sm'\n"
                      << ts_indent() << "           tooltip={{ text: 'Paste' }}\n"
                      << ts_indent() << "         />\n"
                      << ts_indent() << "       </Flex.Item>\n";
        }
...
void t_js_generator::generate_deserialize_map_element(ostream& out, t_map* tmap, string prefix) {
  ...
  // (tmarquesdonascimento): changes to handle complex maps
  indent(out) << prefix << ".push({ key: " << key << ", value: " << val << ", __key__ttype: '" << get_member_type(tmap->get_key_type()) << "', __value__ttype: '" << get_member_type(tmap->get_val_type()) << "' });" << '\n';
}
...
string t_js_generator::ts_get_type(t_type* type) {
  ...
  if (type->is_base_type()) {
    ...
    ts_type = "{ key: " + ktype + ", value: " + vtype + ", __key__ttype: string, __value__ttype: string }[]";
  }
